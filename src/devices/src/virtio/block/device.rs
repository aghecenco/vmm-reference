    ...
    M: GuestAddressSpace + Clone + Send + 'static,
{
        ...
        B::Target: MmioManager<D = Arc<dyn DeviceMmio + Send + Sync>>,
    {
        ...
        // TODO: We don't offer flush (1 << VIRTIO_BLK_F_FLUSH), so writes are faster overall but
        // not guaranteed to be persisted to physical storage in the event of something like power
        // loss. Add this as a configuration option?
        let device_features =
        ...
        // TODO: We're hardcoding the MMIO range len to 4K so assert for this until we decide
        // to do something else.
        assert_eq!(mmio_cfg.range.size(), 0x1000);
        ...
        // TODO: We add the "root=..." option everytime. Add an argument to specifiy whether
        // this is a root device or not? Handle this aspect somewhere else?
        args.common.kernel_cmdline.push_str(&format!(
            " virtio_mmio.device=4K@0x{:x}:{} root=/dev/vda",
            mmio_cfg.range.base().0,
            mmio_cfg.gsi
        ));
...
impl<M: GuestAddressSpace + Clone + Send + 'static> WithDeviceOps for Block<M> {
    ...
    fn activate(&mut self) -> Result<()> {
        ...
        // TODO: Some changes required here to add read-only support as well.
        ...
        let disk =
            StdIoBackend::new(file, self.virtio_cfg.driver_features).map_err(Error::Backend)?;
...
mod tests {
    ...
    use crate::virtio::tests::CommonArgsContext;
    ...
    #[test]
    fn test_device() {
        let mut ctx = CommonArgsContext::new();
        let tmp = TempFile::new().unwrap();

        let common_args = ctx.args();
        let args = BlockArgs {
            common: common_args,
            file_path: tmp.as_path().to_path_buf(),
        };

        let _block = Block::new(args).unwrap();

        // Some quick sanity checks. Most of the functionality around the device should be
        // exercised/validated via integration tests.

        let range = ctx.mmio_cfg.range;

        let bus_range = ctx.mmio_mgr.mmio_device(range.base()).unwrap().0;
